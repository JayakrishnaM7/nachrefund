...
@Component
public class Positionkeeping implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private PositionkeepingRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Positionkeeping.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Positionkeeping.class.getName());
         }else{
