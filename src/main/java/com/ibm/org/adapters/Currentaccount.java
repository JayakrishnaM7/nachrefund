...
@Component
public class Currentaccount implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private CurrentaccountRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Currentaccount.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Currentaccount.class.getName());
         }else{
