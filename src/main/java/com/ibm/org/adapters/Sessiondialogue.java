...
@Component
public class Sessiondialogue implements JavaDelegate{

    @Autowired
    private Utility utility;

    @Autowired
    private SessiondialogueRestProcessor restProcessor;

    private final static Logger LOGGER = Logger.getLogger(Sessiondialogue.class.getName());
    ...
    public void execute(DelegateExecution execution) throws Exception {
         ...
         if(response instanceof ApiError ||  response instanceof GenericError){
            //Failure Path
            throw new RuntimeException("Error occurred in the adapter:"+Sessiondialogue.class.getName());
         }else{
