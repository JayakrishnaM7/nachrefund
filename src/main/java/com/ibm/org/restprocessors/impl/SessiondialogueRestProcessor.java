...
@Service
public class SessiondialogueRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(SessiondialogueRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.sessiondialogue.hosturl}")
	private String url;
	
	@Value("${spring.api.sessiondialogue.context-path}")
	private String contextPath;

	@Value("${spring.api.sessiondialogue.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  SessiondialogueRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ SessiondialogueRestProcessor.class.getName());
