...
@Service
public class AchoperationsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(AchoperationsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.achoperations.hosturl}")
	private String url;
	
	@Value("${spring.api.achoperations.context-path}")
	private String contextPath;

	@Value("${spring.api.achoperations.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  AchoperationsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ AchoperationsRestProcessor.class.getName());
