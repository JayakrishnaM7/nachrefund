...
@Service
public class CurrentaccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CurrentaccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.currentaccount.hosturl}")
	private String url;
	
	@Value("${spring.api.currentaccount.context-path}")
	private String contextPath;

	@Value("${spring.api.currentaccount.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CurrentaccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CurrentaccountRestProcessor.class.getName());
