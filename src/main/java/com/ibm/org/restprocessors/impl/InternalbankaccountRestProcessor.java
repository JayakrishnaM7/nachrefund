...
@Service
public class InternalbankaccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(InternalbankaccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.internalbankaccount.hosturl}")
	private String url;
	
	@Value("${spring.api.internalbankaccount.context-path}")
	private String contextPath;

	@Value("${spring.api.internalbankaccount.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  InternalbankaccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ InternalbankaccountRestProcessor.class.getName());
