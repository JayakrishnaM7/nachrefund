...
@Service
public class PositionkeepingRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(PositionkeepingRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.positionkeeping.hosturl}")
	private String url;
	
	@Value("${spring.api.positionkeeping.context-path}")
	private String contextPath;

	@Value("${spring.api.positionkeeping.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  PositionkeepingRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ PositionkeepingRestProcessor.class.getName());
