package com.ibm.org.restprocessors.impl;

import com.ibm.org.restprocessors.RestApiInterface;
...
@Service
public class CorrespondentbankoperationsRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CorrespondentbankoperationsRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.correspondentbankoperations.hosturl}")
	private String url;
	
	@Value("${spring.api.correspondentbankoperations.context-path}")
	private String contextPath;

	@Value("${spring.api.correspondentbankoperations.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CorrespondentbankoperationsRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CorrespondentbankoperationsRestProcessor.class.getName());
