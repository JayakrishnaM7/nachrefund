...
@Service
public class CorporatecurrentaccountRestProcessor implements RestApiInterface {
	private static final Logger logger = LoggerFactory.getLogger(CorporatecurrentaccountRestProcessor.class);

    @Autowired
	private Utility utility;

	@Value("${spring.api.corporatecurrentaccount.hosturl}")
	private String url;
	
	@Value("${spring.api.corporatecurrentaccount.context-path}")
	private String contextPath;

	@Value("${spring.api.corporatecurrentaccount.jolt-filename}")
	...
	public Object exceptionMessage(Exception e, String requestObj) {//NOSONAR
		...
		if(e instanceof  HttpClientErrorException){
			if(((HttpClientErrorException) e).getStatusCode().equals(HttpStatus.NOT_FOUND) ||
				...
				apiError.setMessage("API called failed in " +  CorporatecurrentaccountRestProcessor.class.getName());
			...
			error.setMessage("Error occurred in "+ CorporatecurrentaccountRestProcessor.class.getName());
